...
class _CartScreenState extends State<CartScreen> {
    ...
    return BlocConsumer<BaseCubit, BaseStates>(
      ...
      builder: (context, state) {
        ...
        return Scaffold(
          ...
          body: cubit.orders.isEmpty ? noOrdersScreen() : ordersList(cubit),
        );
      },
    );
  }
  ...
  Container noOrdersScreen() {
    return Container(
      ...
      child: Center(
        child: LottieBuilder.asset(LottieAsset.noOrders),
      ),
    );
  }

  Widget ordersList(BaseCubit cubit) {
    return Container(
      color: ColorManager.whiteGrey,
  ...
  Widget cartOrderContainer(Order order, BuildContext context, BaseCubit cubit) {
    return Padding(
      padding:
          const EdgeInsets.symmetric(horizontal: AppPadding.p10).copyWith(bottom: AppPadding.p10),
      child: Slidable(
